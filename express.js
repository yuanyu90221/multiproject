...
app.get('/?', function (req, res) {
    ...
    if(req.query.game == undefined){
    	 res.sendfile(__dirname + '/index.html',{'host':host});
    }
...
//選名字	
function pick_name(key){
	var name =['Sophia', 'Jacob', 'Mason', 'Emma', 'William', 'Chloe', 'Noah', 'Ella', 'James', 'Sofia', 'Zoey', 'Joseph'	, 'David', 'Abigail', 'Andrew', 'Michael', 'Daniel', 'Emily', 'Lily', 'Lucy', 'Richard', 'Peter', 'Philip', 'Quentin', 'Britney', 'Alice', 'Shirley', 'Rose', 'Bill', 'Adam','Henry', 'Jordan', 'Barbara', 'Elsa', 'Gloria', 'Kathy', 'Kate', 'Julie', 'Martha', 'Wendy', 'Frances', 'Andy', 'Caspar', 'Duke', 'Gavin', 'Jack', 'Jim'];
	var sum =0;
	var key_sub = key.substring(18, 1);
	for(var i=0;i<key_sub.length;i++){
		var c = key_sub.charCodeAt(i);
		sum += c;
	}
	var p_number = sum % 47;
	return name[p_number];	
}
